...
test('pager model exposes bounded navigation metadata', () => { const result = load().pagerModel(Array.from({ length: 76 }, (_, index) => index), 2, 75); assert.equal(result.first, 76); assert.equal(result.last, 76); assert.equal(result.hasPrevious, true); assert.equal(result.hasNext, false); });
